...
$: python PATH_TO_SCORE_FILE PATH_TO_GROUNDTRUTH_DIR phase
 ...
 -PATH_TO_GROUNDTRUTH_DIR: path to the directory that has the CM protocol and ASV score.
...
# 修改点：asv_key_file 指向 ASVTorch_Kaldi 子目录下的 trial_metadata.txt
...
def eval_to_score_file(score_file, cm_key_file):
    ...
    if len(submission_scores) != len(cm_data_phase):
        print('CHECK: submission has %d of %d expected trials for phase %s.' % (len(submission_scores), len(cm_data_phase), phase))
        exit(1)
